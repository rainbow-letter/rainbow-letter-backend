...
public class PetRequestSnippet {
    ...
    public static final Snippet PET_CREATE_REQUEST = requestFields(
            ...
            fieldWithPath("owner").type(JsonFieldType.STRING)
                    .description("주인을 부르는 호칭")
                    .attributes(constraints("10자 이내")),
            fieldWithPath("personalities").type(JsonFieldType.ARRAY)
                    .description("반려동물 성격")
                    .attributes(constraints("10자 이내 문자열 배열")),
            fieldWithPath("deathAnniversary").type(JsonFieldType.STRING)
                    .description("아이가 떠난 날")
                    .attributes(constraints("yyyy-MM-dd"))
                    .optional(),
            fieldWithPath("image").type(JsonFieldType.NUMBER)
                    .description("반려동물 이미지")
                    .attributes(constraints("이미지 ID"))
                    .optional()
    );
